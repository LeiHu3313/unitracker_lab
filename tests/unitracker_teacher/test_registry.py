...
def test_g1_teacher_and_extreme_rgmt_ids_are_registered_in_source():
    source = (
    ...
    assert 'id="Unitracker_ExtremeRGMT-v0"' in source
    task_dir = (
        Path(__file__).resolve().parents[2]
...
def test_extreme_rgmt_training_entry_point_requires_frozen_inputs():
    source = (Path(__file__).resolve().parents[2] / "scripts/rsl_rl/extreme_rgmt/train_extreme_rgmt.py").read_text(
        encoding="utf-8"
    )
    assert 'parser.add_argument("--mastered-motion", required=True' in source
    assert '"--challenging-motion", required=True' in source
    assert '"--base-checkpoint"' in source and "required=True" in source
